...
- Follow-up on 2026-07-09 closed the Android/patient family consent-history backend contract locally: `GET /api/v1/patients/:id/shares` returns revoked grants for history, and `smoke:workspace-access` now logs in a personal patient account to verify self/dependent profile isolation, dependent creation, doctor share-target lookup, consent create/list/revoke, revoked-history visibility, and denial of workspace-owned patient profiles. Android source/build verification also passed for `DataAccessScreen.kt`.
