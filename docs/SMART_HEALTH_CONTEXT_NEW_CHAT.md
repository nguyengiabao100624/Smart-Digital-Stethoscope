...
- 2026-06-08 platform-admin chrome copy: Web Admin `Layout` no longer displays the legacy default workspace `Smart Health Clinic` / `Phòng khám` in the sidebar footer or topbar for platform admins. Platform admins now see `Nền tảng: Toàn hệ thống`; workspace admins still see their hospital/clinic workspace name and type.
- 2026-06-08 setup-guide readability: `SMART_HEALTH_NEXT_DAY_SETUP_GUIDE.md` was rewritten as a detailed Vietnamese guide with proper accents, covering GitHub Actions, Render env, Firebase Hosting deploy, admin creation, SMTP/SMS/Zalo, Android, ESP32-S3 first flash, cloud OTA, and final smoke checks.
