...
- **OPEN/PHYSICAL+SECURITY:** ADB currently has no attached target, so installation/visual/TalkBack evidence for this APK is unavailable. Production ESP WSS/ACK/audio-v2/scan/OTA/rollback remains a physical gate. The exposed Firebase service-account credential must be rotated without revoking the currently working key until the replacement has deployed and passed Firebase/Render verification. Overall G4 remains **PARTIAL**.
