...
- 2026-07-10 source follow-up closed the missing Shcare Portal appointments/consultations module before ESP32 work: backend `/api/v1/appointments` and `/api/portal/appointments`, repository support, migration `010_appointments.sql`, `/portal/appointments` UI, route/menu, validation, audit/notification side effects, and smoke coverage are in source. Verification passed locally with backend smoke/check/test/repository smoke and Shcare Web lint/typecheck/build/Firebase build. Local Vite is running at `http://127.0.0.1:8080`; migration application still needs `DATABASE_URL`, and live deploy/smoke is pending.
