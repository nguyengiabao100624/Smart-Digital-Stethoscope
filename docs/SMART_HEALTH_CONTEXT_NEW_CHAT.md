...
- Xiaomi reappeared through Wireless ADB. The exact APK was installed with `install -r`, opened in `MainActivity`, rendered a clean 1080x2400 login surface and produced no crash/DNS/SSL/5xx log. The new `DeviceAccessRedeemScreenTest` passed `1/1` physically in `2.345s`, proving opaque-code normalization, exact granted-device navigation, no standalone Device ID field and 48dp scan/submit targets. The test package was removed and the normal App relaunched. TalkBack is currently disabled on the handset, so an assisted authenticated sweep remains separate.
