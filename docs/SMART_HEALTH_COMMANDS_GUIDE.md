...
2026-07-10 live follow-up: Web Admin AI Measurements and Doctor Approval are backed by real scan reprocess, doctor-request, and clinic-catalog contracts, and `smoke:workspace-access` covers scan create/upload/complete/reprocess/delete with selected-scan mutation denials. Repository smokes now cover SQL/runtime parity for approved doctors and approved doctor requests. Use these local checks before deploy:
...
Last verified in this workspace on 2026-07-10 after the Web Admin AI/Doctor Approval/admin mutation closure. `npm.cmd run lint` passed, `npm.cmd run build:firebase:admin` passed, `npm.cmd run smoke:admin-mutation` passed against live Render/Admin with run id `admin-mutation-mre2pt6i`, and Firebase Hosting site `shcare-admin` was deployed as version `projects/162993928259/sites/shcare-admin/versions/c6371f255aa5f85f`. The remaining TanStack build messages are dependency unused-import warnings from `node_modules`; the remaining large `xlsx` asset is a lazy export-library chunk.
...
`smoke:admin-mutation` reads `smart-health-embedded\web-monitor\.test-data\production-role-smoke-credentials.json` by default and uses the `platform` account. Optional overrides are `SMOKE_ADMIN_URL`, `SMART_HEALTH_API_BASE_URL` or `SMOKE_API_BASE_URL`, `SMOKE_CREDENTIALS_FILE`, and `SMOKE_ACCOUNT_KEY`. It does not print passwords or bearer tokens. Coverage includes platform workspace/package/admin-account/patient/device/doctor/scan-audio/reprocess/notification/storage/settings mutations with cleanup, route checks for overview, account, devices, patients, doctors, doctor approval, AI measurements, clinics, packages, notifications, storage, settings, admin accounts, and audit log, plus `/account` assertions for profile/avatar/basic-info UI, password/2FA/session controls, personal notification tab, and backend `/api/auth/sessions`. Last live pass from this workspace used run id `admin-mutation-mre2pt6i`; all checked API responses were 200/201 and cleanup returned HTTP 200 for all 10 created/restored targets.
