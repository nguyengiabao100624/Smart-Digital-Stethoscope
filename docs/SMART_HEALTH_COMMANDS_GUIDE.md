...
`smoke:admin-mutation` reads `smart-health-embedded\web-monitor\.test-data\production-role-smoke-credentials.json` by default and uses the `platform` account. Optional overrides are `SMOKE_ADMIN_URL`, `SMART_HEALTH_API_BASE_URL` or `SMOKE_API_BASE_URL`, `SMOKE_CREDENTIALS_FILE`, and `SMOKE_ACCOUNT_KEY`. It does not print passwords or bearer tokens. Coverage includes platform workspace/package/patient/device/notification/storage/settings mutations with cleanup, route checks for overview, account, devices, patients, clinics, packages, notifications, storage, settings, admin accounts, and audit log, plus `/account` assertions for profile/avatar/basic-info UI, password/2FA/session controls, personal notification tab, and backend `/api/auth/sessions`. Last live pass from this workspace used run id `admin-mutation-mrdgnc3d`; device PATCH returned 200 and all created/restored resources cleaned up with HTTP 200 responses.
