...
function createRuntimeDb() {
  return {
    ...
    users: [
      ...
      { id: ADMIN_ALPHA, role: "workspace_admin", organizationId: ORG_ALPHA, accountStatus: "active" },
      { id: PLATFORM_ADMIN, role: "platform_admin", organizationId: "", accountStatus: "active" },
    ],
    memberships: [
...
function assignIntent(patientId, at) {
  return {
    deviceId: DEVICE_ID,
    operation: "assign",
    actorUserId: ADMIN_ALPHA,
...
function createSqlHarness(options = {}) {
  ...
  return { db, queries, repositories, transaction };
}
...
test("SQL provision consumes the canonical RETURNING device for runtime and caller state", async () => {
  const { db, repositories } = createSqlHarness({
    canonicalDevice: { secretHash: CANONICAL_SECRET_HASH },
  });
