...
- Local PowerShell still does not expose Render CLI or `DATABASE_URL`, so the production DB migration was applied via Supabase rather than the local backend migrator. The live backend code path still needs Git push/Render auto-deploy and post-deploy live smoke before this specific repository path is claimed live.
