...
function testServerProviderSafetyContract() {
  ...
  assert.match(
    functionSource,
    /setCustomUserClaims\(firebaseUser\.uid, provisioningClaims\)[\s\S]*firebaseUser = await firebaseAdminApp\.auth\(\)\.getUser\(firebaseUser\.uid\)[\s\S]*createManagedAdminWithAudit/,
  );
