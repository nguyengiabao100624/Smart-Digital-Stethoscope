...
- Android needs-info UI follow-up: `DoctorApprovalPendingScreen` now exposes `Loại đăng ký` in the request-info form, so an existing account that was misclassified as `Bác sĩ cơ sở` can switch to `Bác sĩ tư` without admin/backend data repair. Selecting solo practice clears the facility id and shows editable `Tên phòng khám tư`; selecting facility keeps the catalog path. The background status poll now stops in `needs_info` to avoid overwriting form edits. Verification passed: Android `.\gradlew.bat :app:compileDebugKotlin` and `.\gradlew.bat :app:assembleDebug`.
