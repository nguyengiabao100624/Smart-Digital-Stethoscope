...
- MIUI still denies shell input injection. The Gradle instrumentation install cleared the phone's prior Firebase session, so the exact signed-in physical dashboard tap remains a visual follow-up after the account signs in again; no password or bearer token was passed through ADB to fabricate that proof.
