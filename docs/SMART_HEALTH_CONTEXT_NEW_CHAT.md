...
- SMTP test email now maps common Gmail/Nodemailer failures to clear 400 messages and uses short connection/socket timeouts so Web Admin does not sit on `Đang gửi...` for a long time. If Gmail test fails, first check `SMTP_USER`, `SMTP_PASS` App Password, and whether `SMTP_FROM` uses the same Gmail account.
