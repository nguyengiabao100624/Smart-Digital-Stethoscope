...
function createRepositories(options) {
    ...
    await withSql((pool) =>
      pool.query(
        ...
        [
          ...
          optionalForeignKey(user.organizationId),
          optionalForeignKey(user.patientId),
    ...
    await withSql((pool) =>
      pool.query(
        ...
        [
          ...
          optionalForeignKey(organization.ownerUserId),
          optionalForeignKey(organization.packageId),
  ...
  async function queryUpsertNotification(queryable, notification) {
    return queryable.query(
        ...
        [
          ...
          optionalForeignKey(notification.campaignId),
  ...
  async function queryUpsertPatient(queryable, patient) {
    ...
    await queryable.query(
        ...
        [
          patient.id,
          optionalForeignKey(patient.organizationId),
          optionalForeignKey(patient.ownerUserId),
  ...
  async function queryUpsertAppointment(queryable, appointment) {
    await queryable.query(
      ...
      [
        appointment.id,
        optionalForeignKey(appointment.organizationId),
        optionalForeignKey(appointment.patientId),
        optionalForeignKey(appointment.doctorUserId),
        optionalForeignKey(appointment.createdByUserId),
        ...
        optionalForeignKey(appointment.rescheduledByUserId),
        optionalTimestamp(appointment.deletedAt),
        optionalForeignKey(appointment.deletedByUserId),
  ...
  async function queryUpsertPatientShare(queryable, grant) {
    return queryable.query(
        ...
        [
          ...
          grant.patientId,
          optionalForeignKey(grant.organizationId),
          ...
          optionalForeignKey(grant.grantedByUserId),
          ...
          optionalForeignKey(grant.revokedByUserId),
          optionalTimestamp(grant.createdAt),
  ...
  async function queryInsertDeviceClaim(queryable, claim) {
    await queryable.query(
      ...
      [
        claim.id,
        claim.deviceId,
        optionalForeignKey(claim.organizationId),
        claim.claimCodeHash,
        optionalForeignKey(claim.createdByUserId),
        optionalForeignKey(claim.claimedByUserId),
        claim.expiresAt,
        optionalTimestamp(claim.claimedAt),
        optionalTimestamp(claim.revokedAt),
        optionalForeignKey(claim.revokedByUserId),
  ...
  async function upsertScanSql(scan) {
    ...
    await withSql((pool) =>
      pool.query(
        ...
        [
          scan.id,
          optionalForeignKey(scan.organizationId),
          scan.patientId,
          optionalForeignKey(scan.deviceId),
          optionalForeignKey(scan.createdByUserId),
          ...
          optionalForeignKey(scan.audioFileId),
          optionalForeignKey(scan.aiResultId),
          JSON.stringify(encrypted ? phiPayload : {}),
        ]
      )
    );
  ...
  async function queryUpsertDeviceCommand(queryable, command) {
    await queryable.query(
        ...
        [
          command.id,
          command.deviceId,
          optionalForeignKey(command.organizationId),
          optionalForeignKey(command.requestedByUserId),
          ...
          command.issuedAt,
          command.expiresAt,
          optionalTimestamp(command.executionExpiresAt),
          command.acceptedAt || command.issuedAt,
          optionalTimestamp(command.queuedAt),
          optionalTimestamp(command.deliveredAt),
          optionalTimestamp(command.acknowledgedAt),
          optionalTimestamp(command.applyingAt),
          optionalTimestamp(command.appliedAt),
          optionalTimestamp(command.failedAt),
          optionalTimestamp(command.expiredAt),
  ...
  async function queryEnsureRoleRequestWorkspace(queryable, workspace) {
    ...
    await queryable.query(
      ...
      [
        ...
        optionalForeignKey(workspace.ownerUserId),
        optionalForeignKey(workspace.packageId),
    ...
    async createWithAudit(input, auditInput = {}, idempotencyInput = null, responseStatus = 201) {
      ...
      const result = await withSqlTransaction(async (client) => {
        ...
        const inserted = await client.query(
          ...
          [
            exportJob.id,
            optionalForeignKey(exportJob.organizationId),
            optionalForeignKey(exportJob.createdByUserId),
            exportJob.format,
            exportJob.dataset,
            exportJob.scopeKind,
  ...
  async function insertPatientImportBatchSql(client, batch) {
    await client.query(
      ...
      [
        batch.id,
        optionalForeignKey(batch.organizationId),
        optionalForeignKey(batch.actorUserId),
        ...
        const sqlDevice = await withSql(async (pool) => {
          const result = await pool.query(
            `
              ...
              RETURNING *
            `,
            [id, token, optionalForeignKey(workspaceId), optionalForeignKey(authSessionId)],
          );
          return result.rows[0] ? rowToNotificationDevice(result.rows[0]) : null;
        });
