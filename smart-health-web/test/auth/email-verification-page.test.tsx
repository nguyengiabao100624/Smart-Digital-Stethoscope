...
vi.mock("../../src/lib/firebase-client", () => ({
  refreshFirebaseVerification: (...args: unknown[]) => refreshVerification(...args),
}));
...
describe("EmailVerificationPage", () => {
  ...
  it("keeps the user in a recoverable pending state until Firebase confirms verification", async () => {
    refreshVerification.mockResolvedValue({ verified: false, idToken: "token" });
    renderPage();
    ...
    expect(
      await screen.findByText("Email chưa được xác minh. Mở liên kết trong hộp thư rồi kiểm tra lại."),
    ).toBeVisible();
