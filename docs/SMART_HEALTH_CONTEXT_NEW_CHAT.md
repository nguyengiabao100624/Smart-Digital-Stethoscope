...
- MIUI still denies shell input injection, so ADB could not fabricate a tap on the signed-in dashboard. The installed app and retained doctor session were captured, while the view/provision/release behavior was verified through route tests, backend positive/negative security tests and on-device Compose instrumentation.
