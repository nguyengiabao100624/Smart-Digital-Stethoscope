...
- Supabase project `mahvymyncxszvuhlycwp` is the active PostgreSQL/S3 authority. Migrations `001-056` pass on the production candidate. The PostgreSQL hot path no longer writes a full runtime snapshot for each authenticated/device read, and the notification worker no longer crashes on an unhandled push-provider rejection.
- Platform Admin is live on Firebase Hosting version `79f1596432a2d549`; Shcare Public/Workspace Portal is live on version `554570a43b88f25d`. Current production public, portal and role smokes pass. The full Admin mutation journey passes, including workspace/admin/profile/password/package/storage/patient/staff/settings operations and deterministic cleanup; every cleanup receipt returned HTTP 200.
- The current production-debug Android APK `1.0.0-rc.2` is installed on Xiaomi. SHA-256 is `F4FEA79E559377FF8FD9A7D89B4C7142084E76C336D3230F5112145F2D6D6814`. Before instrumentation, a cold start loaded the authenticated doctor dashboard and current workspace from production with no crash or server-connection error. JVM tests are `860/860`, lint reports no issues, and device instrumentation completed `96` tests with `0` failures and `8` explicitly gated HIL skips. Instrumentation removed the main package as part of cleanup; the same APK was reinstalled successfully and now opens the login screen without a backend error.
- Firmware production and OTA environments build successfully. Current generic artifacts are `C731711F7B5DD47B54A7CBDEECC0D4C8D8BD3D63414DC5EF3D73E2B8E89C3E82` and `D495357D5B2A15AA26467A66BCA6E0EA6AC366FF9E8C905E2EBAD916F66F0B6D`. COM9 is present. The retained external HIL header was updated from the retired Render hostname to `shcare-api-prod.onrender.com`, the development environment was cleaned to prevent stale-object reuse, and firmware `54337CE5A6759867DF211EE100A590442D513338EA65496C09E6ED146BBC7D87` was wired-flashed with esptool hash verification, without full erase or eFuse changes.
- Post-flash serial proves both I2S microphones still produce non-zero samples. The board has a saved Wi-Fi configuration but association currently fails with reason `201 NO_AP_FOUND`; the Xiaomi is on the 2.4 GHz network `Gia_Bao`. Production metrics therefore correctly remain `smart_health_devices_online 0`. G4 cloud lanes are deployed and verified, but full G4 remains **PARTIAL, not PASS** until the target-network password is entered through the App's secure field and the current production WSS/ACK/audio-v2/durable-scan/signed-OTA/forced-rollback chain passes.
...
## 2026-08-28 G3 production WSS enrollment checkpoint

- Supabase project `mahvymyncxszvuhlycwp` now contains the disposable, audited factory fixture `shcare-g3-prod-demo` in `org_default_clinic` with ownership `provisioned` and firmware `1.0.2-prod-demo`. Raw credential material remains outside Git and documentation.
