...
- Local strict production gate still reports `BLOCKED`; the email-specific warning is expected locally because `BREVO_API_KEY` and `BREVO_FROM_EMAIL` are not in local PowerShell. For real inbox delivery on Render, set/verify `EMAIL_PROVIDER=brevo`, `BREVO_API_KEY`, `BREVO_FROM_EMAIL`, optional `BREVO_FROM_NAME`, and `WEB_PORTAL_URL=https://shcare.web.app`, then redeploy/poll the endpoint.
