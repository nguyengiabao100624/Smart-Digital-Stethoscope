...
- 2026-06-12 update: Android needs-info resubmit now also shows a manual `Loại đăng ký` selector. This covers existing doctor accounts that were already stored as facility doctors: the user can switch to `Bác sĩ tư`, the form changes from `Cơ sở y tế` to editable `Tên phòng khám tư`, and resubmit sends the solo-practice account type. The polling loop stops while status is `needs_info` so the form no longer overwrites in-progress edits.
