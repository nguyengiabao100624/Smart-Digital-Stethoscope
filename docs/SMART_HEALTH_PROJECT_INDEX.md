...
- Follow-up on 2026-07-09 closed a Shcare Portal workspace summary source gap locally: `/portal/workspace` no longer uses hardcoded zero counters, backend `/me` returns operational summaries for current workspace and memberships, and `smoke:workspace-access` plus local `SMOKE_DISABLE_WEB_SECURITY=1 bun run smoke:portal-browser` cover the contract. This source slice has not yet been deployed to Render/Firebase; do not call the live site production-complete for this contract until backend/web deploy and live `bun run smoke:portal-browser` pass without the local CORS bypass.
...
- Deploy and live-smoke the workspace-summary source slice before calling it production-complete: backend deploy, Shcare Web deploy, then live `bun run smoke:portal-browser` without `SMOKE_DISABLE_WEB_SECURITY`.
